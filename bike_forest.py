...
grid_search = GridSearchCV(pipeline, parameters, cv=folds, n_jobs=-1, verbose=0)

# search the grid for the best the model
grid_search.fit(X.iloc[trainA_idx, :], Y.iloc[trainA_idx, :])
model = grid_search.best_estimator_

# forecast
predict = model.predict(X.iloc[trainB_idx, :])
